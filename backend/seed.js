// ============================================================
//  GourmetGo Seed Script
//  Run: node seed.js
//  Seeds canteens, vendors, menu items, and test students
// ============================================================
...
async function seed() {
  try {
    console.log('🌱 Seeding GourmetGo database...\n');

    // ── CANTEENS (IDs will be 1–4) ─────────────────────────────
    await db.query(`
    ...
    `);
    console.log('✅ Canteens inserted');

    // ── VENDORS (matching mock data credentials) ────────────────
    const vp = await bcrypt.hash('bakery123', 10);
    const v2 = await bcrypt.hash('michaels123', 10);
    const v3 = await bcrypt.hash('mingos123', 10);
    const v4 = await bcrypt.hash('fresh123', 10);
    await db.query(`
      INSERT INTO vendors (name, email, password_hash, canteen_id) VALUES
      ('Ravi Kumar',      'bakery@christuniversity.in',    ?, 1),
      ("Michael D''Souza",'michaels@christuniversity.in',  ?, 2),
      ('Priya Nair',      'mingos@christuniversity.in',    ?, 3),
      ('Anita Sharma',    'freshateria@christuniversity.in',?, 4)
    `, [vp, v2, v3, v4]);
    console.log('✅ Vendors inserted');

    // ── MENU ITEMS (item_ids match mock data exactly) ───────────
    // Christ Bakery — items 101-117
    await db.query(`
    ...
    // Michael's Corner — items 201-215
    await db.query(`
      INSERT INTO menu_items (item_id, canteen_id, name, price, category, is_veg, prep_time_mins, daily_stock_limit, stock_remaining, image_url) VALUES
    ...
    `);
    console.log('✅ Menu items inserted (62 items across 4 canteens)');

    // ── TEST STUDENTS ──────────────────────────────────────────
    const sp = await bcrypt.hash('student2024', 10);
    await db.query(`
      INSERT INTO students (name, email, password_hash, roll_number, department, year_of_study, loyalty_points, wallet_balance) VALUES
      ('Aarav Sharma',   'aarav.sharma@christuniversity.in',   ?, 'CHR2024CS001', 'Computer Science',    2, 340, 500),
      ('Priya Krishnan', 'priya.k@christuniversity.in',        ?, 'CHR2024EC002', 'Electronics',         3, 120, 300),
      ('Rohan Mathew',   'rohan.m@christuniversity.in',        ?, 'CHR2024ME003', 'Mechanical Engg',     1,  50, 200),
      ('Sneha Iyer',     'sneha.iyer@christuniversity.in',     ?, 'CHR2024BA004', 'Business Admin',      2, 200, 400),
      ('Karthik Nair',   'karthik.n@christuniversity.in',      ?, 'CHR2024CS005', 'Computer Science',    3,  80, 150),
      ('Divya Pillai',   'divya.p@christuniversity.in',        ?, 'CHR2024PS006', 'Psychology',          2, 500, 600),
      ('Arun Varghese',  'arun.v@christuniversity.in',         ?, 'CHR2024CE007', 'Civil Engg',          4,  30, 100),
      ('Meera Thomas',   'meera.t@christuniversity.in',        ?, 'CHR2024EN008', 'English Literature',  1, 150, 250)
    `, Array(8).fill(sp).flat());
    console.log('✅ 8 test students inserted');
    ...
    console.log('  VENDORS');
    console.log('  bakery@christuniversity.in     / bakery123    (Christ Bakery)');
    console.log('  michaels@christuniversity.in   / michaels123  (Michael\'s Corner)');
    console.log('  mingos@christuniversity.in     / mingos123    (Mingos)');
    console.log('  freshateria@christuniversity.in/ fresh123     (Freshateria)\n');
    console.log('  STUDENTS  (all use password: student2024)');
    console.log('  aarav.sharma@christuniversity.in   — 340 loyalty pts, ₹500 wallet');
    console.log('  priya.k@christuniversity.in        — 120 loyalty pts, ₹300 wallet');
    console.log('  rohan.m@christuniversity.in        —  50 loyalty pts, ₹200 wallet');
    console.log('  sneha.iyer@christuniversity.in     — 200 loyalty pts, ₹400 wallet');
    console.log('  karthik.n@christuniversity.in      —  80 loyalty pts, ₹150 wallet');
    console.log('  divya.p@christuniversity.in        — 500 loyalty pts, ₹600 wallet');
    console.log('  arun.v@christuniversity.in         —  30 loyalty pts, ₹100 wallet');
    console.log('  meera.t@christuniversity.in        — 150 loyalty pts, ₹250 wallet');
    console.log('══════════════════════════════════════════════════════\n');
