...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [[{ revenue }]] = await db.query('SELECT COALESCE(SUM(p.amount), 0) as revenue FROM payments p WHERE DATE(p.paid_at) = ? AND p.status = "completed"', [today]);
    ...
    res.json({ total_orders, revenue, active_canteens, open_grievances, pending_refunds, avg_wait_time: '8 mins' });
  } catch (err) { res.status(500).json({ error: err.message }); }
};

exports.getAllOrders = async (req, res) => {
  try {
    const { canteen_id, status, date } = req.query;
    let query = `SELECT o.*, c.name as canteen_name, s.name as student_name FROM orders o JOIN canteens c ON o.canteen_id = c.canteen_id JOIN students s ON o.student_id = s.student_id WHERE 1=1`;
    const params = [];
...
exports.updateCanteen = async (req, res) => {
  try {
    ...
    await db.query('UPDATE canteens SET name=COALESCE(?,name), description=COALESCE(?,description), operating_hours=COALESCE(?,operating_hours), status=COALESCE(?,status), banner_image=COALESCE(?,banner_image) WHERE canteen_id=?',
  ...
  try {
    const [grievances] = await db.query(
      `SELECT g.*, s.name as student_name, c.name as canteen_name,
       ...
       FROM grievances g JOIN students s ON g.student_id = s.student_id JOIN canteens c ON g.canteen_id = c.canteen_id
...
exports.replyGrievance = async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO notifications (user_id, user_role, type, message) VALUES (?, "student", "grievance_update", ?)', [g.student_id, `📋 Admin replied to your grievance ${g.ticket_code}`]);
...
exports.getAllRefunds = async (req, res) => {
  try {
    const [refunds] = await db.query(
      `SELECT r.*, s.name as student_name, o.order_code FROM refunds r
       JOIN students s ON r.student_id = s.student_id JOIN orders o ON r.order_id = o.order_id
...
exports.processRefund = async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO notifications (user_id, user_role, type, message) VALUES (?, "student", "refund_processed", ?)', [refund.student_id, `💰 Refund of ₹${refund.amount} credited to your GourmetWallet!`]);
...
exports.getAnalytics = async (req, res) => {
  try {
    ...
    const [canteenStats] = await db.query(
      `SELECT c.canteen_id, c.name, c.avg_rating, COUNT(o.order_id) as total_orders, COALESCE(SUM(o.total_amount),0) as revenue
