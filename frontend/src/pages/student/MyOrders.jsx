...
import { ChevronDown, ChevronUp, QrCode, Repeat, Star, AlertCircle, RotateCcw } from 'lucide-react';
...
const STATUS_IDX = { placed: 0, accepted: 1, preparing: 2, ready: 3, picked_up: 4, cancelled: -1 };
      ...
      {STEPS.map((step, i) => (
        <div key={step.key} className="flex items-center flex-1">
          <div className={`flex flex-col items-center flex-shrink-0 ${i <= idx ? 'opacity-100' : 'opacity-30'}`}>
            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm transition-all ${i <= idx ? 'border-gold-500 bg-gold-500/20' : 'border-white/20 bg-white/5'} ${i === idx ? 'animate-pulse-gold' : ''}`}>
              {step.icon}
            </div>
...
export default function MyOrders() {
  ...
  const active = myOrders.filter(o => !['picked_up', 'cancelled'].includes(o.status));
  const history = myOrders.filter(o => ['picked_up', 'cancelled'].includes(o.status));
  ...
  const simAdvance = (order) => {
    const next = { placed: 'accepted', accepted: 'preparing', preparing: 'ready', ready: 'picked_up' };
    if (next[order.status]) {
      updateOrderStatus(order.order_id, next[order.status]);
      toast(`Order status updated to: ${next[order.status].replace('_', ' ')}`, 'order');
    }
  };
